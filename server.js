...
const bodyParser = require('body-parser'); // bodyParser is often not needed if you use express.json() and express.urlencoded()

const pgSession = require('connect-pg-simple')(session);

const pool = require('./src/server/config/db');
const { ADMIN_JWT_SECRET, USER_JWT_SECRET, SESSION_SECRET } = require('./src/server/config/secrets');

const authRoutes = require('./src/server/routes/authRoutes');
const adminAuthRoutes = require('./src/server/routes/adminAuthRoutes');
const userRoutes = require('./src/server/routes/userRoutes');
const jobRoutes = require('./src/server/routes/jobRoutes');

// --- NEW IMPORTS: Authentication and Authorization Middleware for Admin Endpoints ---
const authenticateAdminJWT = require('./src/server/middleware/authenticateAdminJWT');
const authorizeAdmin = require('./src/server/middleware/authorizeAdmin');
...
app.prepare().then(async () => {
    ...
    // 💡 CRITICAL ADDITION: Trust the reverse proxy (Plesk) for correct protocol detection
    server.set('trust proxy', 1);

    server.use(express.static(path.join(process.cwd(), 'public')));

    // 💡 Moved express.json() to a global position
    // This parses JSON body for all incoming requests
    server.use(express.json());

    const frontendOrigin = dev
        ? `http://localhost:3000` // Assuming your Next.js dev server runs on port 3000
        : 'https://upbeat-swanson.217-154-37-86.plesk.page'; // Your actual production frontend URL

    server.use(cors({
        origin: frontendOrigin, // This will now correctly match your frontend's origin
        credentials: true,
        methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'], // Explicitly allow common methods
        allowedHeaders: ['Content-Type', 'Authorization'] // Include if you use Authorization header
    }));
    ...
    server.use(session({
        ...
        cookie: {
            httpOnly: true,
            secure: process.env.NODE_ENV === 'production', // This should now correctly be true in production due to 'trust proxy'
            maxAge: 24 * 60 * 60 * 1000,
            sameSite: process.env.NODE_ENV === 'production' ? 'None' : 'Lax' // This should correctly be 'None' in production
        }
    }));

    // Routes (express.json() is now global, so removed from individual route definitions if present)
    server.use('/api/auth', authRoutes); // express.json() is handled globally now
    server.use('/api/admin', adminAuthRoutes); // express.json() is handled globally now
    server.use('/api/user', userRoutes);
    server.use('/api/job', jobRoutes); // express.json() is handled globally now
    ...
    server.put('/api/admin/user/:userId', authenticateAdminJWT, authorizeAdmin(['super_admin', 'admin']), async (req, res) => {
        const { userId } = req.params;
        const {
